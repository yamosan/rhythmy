...
function sketch(p: p5) {
  ...
  function setupUI() {
    animUI = new AnimationsUI(p, store.cells, leftMargin, topMargin, cellSize, currentStep, [animations[store.id]])
...
function defineSketch(socket: Sock, initData: { notes: binary[], id: number }) {
  sock = socket
  defineReceiver()
