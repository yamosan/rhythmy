...
class NotesUI extends Grid {
  ...
  onClick(mouseX: number, mouseY: number): [x: number, y: number] {
    const xCondition =
      this.ox < mouseX && mouseX < this.ox + this.cellSize * this.row;
    const yCondition =
      this.oy < mouseY && mouseY < this.oy + this.cellSize * this.column;
